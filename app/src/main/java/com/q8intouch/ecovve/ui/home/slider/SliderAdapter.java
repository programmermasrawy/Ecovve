...
public class SliderAdapter extends RecyclerView.Adapter<SliderCard> {
    ...
    private List<EcovveAllHabits.Data> suborders;
    //    private final View.OnClickListener listener;
    Context context;
    ControlsListeners listeners;

    public SliderAdapter(List<EcovveAllHabits.Data> suborders, int count, Context context, ControlsListeners listeners) {
    ...
    public SliderAdapter(int count, ControlsListeners listeners) {
        this.count = count;
        this.listeners = listeners;
    }
    ...
    public void onBindViewHolder(SliderCard holder, final int position) {
        if (suborders != null) {
            ...
            if (!suborders.get(position).getBrand().getLogo().isEmpty())
                if (!suborders.get(position).getBrand().getLogo().toString().contains("http"))
            ...
            if (suborders.get(position).getImage().size() != 0)
                if (!suborders.get(position).getImage().get(0).contains("http"))
                    Glide.with(context).load(URLs.IMAGES_URL + suborders.get(position).getImage().get(0).toString()).into(holder.cover);
                else
                    Glide.with(context).load(suborders.get(position).getImage().get(0).toString()).into(holder.cover);
